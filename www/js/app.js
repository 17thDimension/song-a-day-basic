...
(function() {
  angular.module("songaday").controller("AppCtrl", function($sce, SongService, AccountService, $state, $rootScope, $scope, $ionicSideMenuDelegate, $stateParams, $timeout, ListenService, AudioVisualizerService, $window) {
    ...
    $rootScope.showNotification = function(notice) {
      ...
      _($rootScope.notifications).each(function(note){
        if (songID === notice.link){
          $rootScope.notifications.$remove(note)
        }
      })
...
(function() {
  angular.module("songaday").controller("ChatCtrl", function($scope, AccountService,SongService,Auth) {
    var chatRef = new Firebase('https://song-a-day.firebaseio.com/chat');
    var chat = new FirechatUI(chatRef, document.getElementById('firechat-wrapper'));
      AccountService.refresh(function(myself){
        chat.setUser(myself.user_id, myself.alias);
      });
  });
...
(function() {
  angular.module("songaday").controller("RecordCtrl", function($rootScope, $scope, $state, SongService, $window, AccountService, $stateParams, TransmitService, AudioContextService, RecordService, MultiTrackService, $ionicModal, BetaService, AudioVisualizerService) {
    ...
    TransmitService.lastTransmission(function(song) {
      ...
      today = new Date();
      if (today.getDay() === latest_date.getDay() && today.getYear() === latest_date.getYear() && today.getMonth() === latest_date.getMonth()) {
        $scope.song = song;
  ...
  angular.module("songaday").factory("SongService", function($firebaseObject, $firebaseArray, AccountService, FBURL) {
    ...
    return {
      ...
      comment: function(song, comment) {
        ...
        notification = {};
        comments.$loaded(function(comnts){
          uniq_comments = _.uniq(comnts,'author.key')
          console.log(uniq_comments,"UNIQUE!")
          for (var com in uniq_comments) {
            if (comnts[com] && comnts[com].author){
              var commentor_key = comnts[com].author.key
              if (commentor_key == comment.author.key){
                return;
              }
            }else{
              return
            }
            console.log('the comment!',com)
            $firebaseArray(new Firebase(FBURL + 'notices/' + commentor_key)).$loaded(function(commented_notices){
              var commentor_notice = {};
              commentor_notice.message = comment.author.alias + ' also commented on ' + song.title + " ~ "+ comment.comment;
              commentor_notice.link = song.$id;
              commentor_notice.author = comment.author;
              commented_notices.$add(commentor_notice);
            })
